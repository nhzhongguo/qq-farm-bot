  ...
  <div class="ds-page space-y-5">
    ...
    <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <div class="ds-surface p-4">
        ...
        <div class="mt-2 truncate text-lg font-semibold">
          {{ currentAccount?.nick || currentAccount?.name || '未选择账号' }}
        </div>
      </div>
      <div class="ds-surface p-4">
        <div class="text-sm text-[var(--color-text-secondary)]">
      ...
      <div class="ds-surface p-4">
        ...
        <div class="mt-2 text-lg font-semibold" :class="activeTaskCount ? 'text-emerald-600 dark:text-emerald-400' : ''">
          {{ activeTaskCount }}
        </div>
      </div>
    </section>
          ...
          <div v-for="task in scope.tasks" :key="task.name" class="grid gap-2 px-4 py-3 text-sm md:grid-cols-[minmax(0,1.4fr)_repeat(4,minmax(0,1fr))]">
            <div class="min-w-0">
              <div class="truncate font-medium">
                {{ task.name }}
              </div><div class="text-xs text-[var(--color-text-secondary)]">
                {{ task.kind === 'interval' ? '周期任务' : '一次任务' }} · 间隔 {{ formatDuration(task.delayMs) }}
              </div>
            </div>
    ...
    <section class="space-y-3">
      ...
      <div v-else-if="taskRuns.length" class="ds-surface overflow-hidden">
        <div class="grid-cols-[minmax(0,1.3fr)_minmax(7rem,0.8fr)_minmax(5rem,0.55fr)_minmax(9rem,0.9fr)_minmax(6rem,0.6fr)] hidden gap-3 border-b border-[var(--color-border-default)] px-4 py-2 text-xs text-[var(--color-text-secondary)] font-medium md:grid">
          <span>任务</span>
          <span>触发方式</span>
          <span>状态</span>
          <span>开始时间</span>
        ...
        <div class="divide-y divide-[var(--color-border-default)]">
          <div v-for="run in taskRuns" :key="run.id" class="grid gap-2 px-4 py-3 text-sm md:grid-cols-[minmax(0,1.3fr)_minmax(7rem,0.8fr)_minmax(5rem,0.55fr)_minmax(9rem,0.9fr)_minmax(6rem,0.6fr)] md:gap-3">
            <div class="min-w-0">
              <div class="truncate font-medium" :title="getTaskLabel(run.taskName)">
                {{ getTaskLabel(run.taskName) }}
              </div>
              <div v-if="run.error" class="mt-1 break-words text-xs text-red-600 dark:text-red-400" :title="run.error">
                {{ run.error }}
              </div>
            </div>
    ...
    <p v-if="refreshedAt" class="text-right text-xs text-[var(--color-text-secondary)]">
      最近刷新：{{ formatDate(refreshedAt) }} · 每 15 秒自动更新
    </p>
  </div>
</template>
